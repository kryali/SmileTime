...
////end of checking termianl input!*********************************
    ...
    /* For this version, we'll be able to save and disable hardware acceleration */
    if(hwaccel)//global variable initialized to 0, to set input -w in terminal
     if ( ! getenv("SDL_VIDEO_YUV_HWACCEL") ) {
        putenv("SDL_VIDEO_YUV_HWACCEL=0");
     }
     

   //SDL_VideoDriverName(char* myBuffer, maxSizeToFill) - returns the name of the
   //video driver.
    if (SDL_VideoDriverName(driver, sizeof(driver))) {
	printf("Video driver: %s\n", driver);
    }
   /*Returns a pointer to information about the current video hardware.
   SDL_SetVideoMode(...) SHOULD be called before this funciton, and is not!!!*/
   ...
   /*
      ...
      video_mem - Total amount of video memory in Kilobytes, should be accessed only if hw_available == 1, otherwise it is equal to 0
      vfmt - Pixel format of the video device
      current_w, current_h - Width and height of the current video mode, or of the desktop mode if SDL_GetVideoInfo was called before SDL_SetVideoMode (available since SDL 1.2.10) */
   ...
   //the videoDevice is set here defaultly, but if the user enters
   // ./luvcview -d "nameOfDevice
   ...
   if (avifilename == NULL || *avifilename == 0) {
      ...
      //END rchecking what the vdieo device supports.
     
   
   //calloc(num, size) creates mem for num spaces of size "size".
   ...
   //initialization!!!!!!!!!!!!!!!!!*******************************
   /*
   * Most of the work is done here, the device talked to, and given
   * his buffers.
   */
   if (init_videoIn	(videoIn, (char *) videodevice, 
         width, height, fps, format, grabmethod, avifilename) < 0)
	      exit(1);
   //*******************************************
...
///END of checking user manual INPUT**************************************	
   ...
   pscreen = SDL_SetVideoMode(videoIn->width, videoIn->height + 32, 0,
			 SDL_VIDEO_Flags);
   overlay =
   ...
   creatButt(videoIn->width, 32);//gui.c
   
printf("heyyyyyyyyyyyyyyyyyy\n\n\n");
   SDL_LockYUVOverlay(overlay);
   //p = (unsigned char *) overlay->pixels[0];
   //YUYVbutt is created in gui.c from the call initLut();
...
printf("signalquit = %i\n",videoIn->signalquit);
      while (videoIn->signalquit) {
         ...
         if (videoIn->toggleAvi)
	      ...
	      SDL_LockYUVOverlay(overlay);//LOCK
	         memcpy(p, videoIn->framebuffer,
	         videoIn->width * (videoIn->height) * 2);
	      SDL_UnlockYUVOverlay(overlay);//UNLOCK
...
static int eventThread(void *data)
{
   ...
   while (videoIn->signalquit) {
	   ...
	   frmrate = gdata->frmrate;
            ...
            switch (sdlevent->type) {
	         ...
	         case SDL_VIDEORESIZE:
		         ...
		         pscreen = SDL_SetVideoMode(sdlevent->resize.w,
				      sdlevent->resize.h, 0,SDL_VIDEO_Flags);
	      ...
	      }//end if poll (inner while loop)

	   SDL_UnlockMutex(affmutex);//UNLOCK
	...
	case A_RECORD_TOGGLE:
		SDL_Delay(200);//here he's setting toggleAvi on to start recording an avi file
