...
//device = name of device i.e. "/dev/video0"
//vd is a pointer to allocated memory (garbage passed as videoIn)
...
printf("\ncheck_videoIn\n");
	vd->videodevice = (char *) calloc(16, sizeof(char));
 	snprintf(vd->videodevice, 12, "%s", device);
   printf("video11111111111 %s \n", vd->videodevice);
	     ...
	     int format, int grabmethod, char *avifilename)
{
   ...
   if (grabmethod < 0 || grabmethod > 1)
    ...
    vd->getPict = 0;
    vd->signalquit = 1;
printf("DICKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK\n");
    vd->width = width;
    vd->height = height;
    vd->fps = fps;
    vd->formatIn = format;
   ...
   switch (vd->formatIn) {
      ...
      default:	printf(" should never arrive exit fatal !!\n");
	      goto error;
...
static int init_v4l2(struct vdIn *vd)
{
   ...
   //here we are setting vd->fd to be the file descriptor for the camera.
    if ((vd->fd = open(vd->videodevice, O_RDWR)) == -1) {
	      perror("ERROR opening V4L interface \n");
	   exit(1);
    }

   //setting the memory for vd->cap a v4l2_capability structure
    memset(&vd->cap, 0, sizeof(struct v4l2_capability));
    
   /*This call will fill the vd->cap structure, which contains info about the
   * device, like the name of the driver, the name of the device and the location
   * of the device in the system
   */
   ret = ioctl(vd->fd, VIDIOC_QUERYCAP, &vd->cap);
   //Example: printf("1.%s\n2.%s\n3.%s\n", vd->cap.driver, vd->cap.card, vd->cap.bus_info);
   if (ret < 0) {
	   printf("Error opening device %s: unable to query device.\n",
	   vd->videodevice);
	   goto fatal;
   }
//printf("capabilities:%x\n %x",vd->cap.capabilities, V4L2_CAP_VIDEO_CAPTURE);
   if ((vd->cap.capabilities & V4L2_CAP_VIDEO_CAPTURE) == 0) {
	   printf("Error opening device %s: video capture not supported.\n",
	      vd->videodevice);
	   goto fatal;
   ...
   /*This call fills the fmt struct (v4l2_format), setting the request
      field to VIDIOC_S_FMT tells the device to check and adjust the parameters
      inside vd->fmt against hardware abilities.
   */
   ...
   for (i = 0; i < vd->rb.count; i++) {
      ...
      //the rest of the fields for the v4l2_buffer will be filled by the device.
...
/*
* Video streaming is turned on in this function. Called from 
* the uvcGrab function.
*/
static int video_enable(struct vdIn *vd)
{
   ...
   //note that this will enable the camera to turn on, and 
   //that it will only be enabled if there is at least one buffer 
   //waiting in the queue.
   /***Possible problem: I belive that when when setting up the buffers, 
      only one was being made, and we want more than one if possible.***/
...
#define HEADERFRAME1 0xaf
   ...
   switch (vd->formatIn) {
      ...
      /*	    if(debug)*/
         ...
         //recall - framebuffer = sizeof(width*(height+8)*2)
